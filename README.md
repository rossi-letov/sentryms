...
## Description
This repo contains a Sentryms to help developers build [apps for Zendesk products](https://developer.zendesk.com/apps/docs/apps-v2/getting_started).
...
2. Change (`cd`) into the `app_sentryms` directory
3. Run `npm install`
...
_Note: The App Sentryms currently depends on zat v1.35.12 or greater._
...
## But why?
The App Sentryms includes many features to help you maintain and scale your app. Some of the features provided by the App Sentryms are listed below. However, you don't need prior experience in any of these to be able to use the Sentryms successfully.

- [ES6 (ES2015)](https://babeljs.io/docs/learn-es2015/)

ECMAScript 6, also known as ECMAScript 2015, is the latest version of the ECMAScript standard. The App Sentryms includes the [Babel compiler](https://babeljs.io/) to transpile your code to ES5. This allows you to use ES6 features, such as classes, arrow functions and template strings even in browsers that haven't fully implemented these features.
...
The folder and file structure of the App Sentryms is as follows:

| Name                                    | Description                                                                                  |
|:----------------------------------------|:---------------------------------------------------------------------------------------------|
| [`dist/`](#dist)                        | The folder in which webpack packages the built version of your app                           |
| [`lib/`](#lib)                          | The folder in which the shims and files that make the Sentryms work live                     |
...
#### src
The src directory is where your raw source code lives. The App Sentryms includes different directories for JavaScript, stylesheets, templates and translations. Most of your additions will be in here (and spec, of course!).
...
## Initialization
The App Sentryms's initialization code lives in [`src/index.js`](https://github.com/zendesk/app_sentryms/blob/master/src/javascripts/index.js). For more information, see [inline documentation](https://github.com/zendesk/app_sentryms/blob/master/src/javascripts/index.js).

## API Reference
The App Sentryms provides some classes under `/lib` to help building apps.

### I18n
The I18n (internationalization) module provides a `t` method and Handlebars helper to look up translations based on a key. For more information, see [Using the I18n module](https://github.com/zendesk/app_sentryms/blob/master/doc/i18n.md).

### Storage
The Storage module provides helper methods to interact with `localStorage`. For more information, see [Using the Storage module](https://github.com/zendesk/app_sentryms/blob/master/doc/storage.md).

### View
The View module provides methods to simplify rendering Handlebars templates located under the templates folder. For more information, see [Using the View module](https://github.com/zendesk/app_sentryms/blob/master/doc/view.md).
...
If you're starting a v2 app from scratch you will need to check out the [from-scratch](https://github.com/zendesk/app_sentryms/tree/from-scratch) branch:
...
The from-scratch branch uses up-to-date versions of the libraries included with the App Sentryms and also removes the shims needed when migrating from v1. It also includes sample code to help you get started on v2.
...
If you want to see the exact differences between the master and from-scratch branches click [here](https://github.com/zendesk/app_sentryms/compare/from-scratch).
...
To resolve this problem, set default values for parameters or create a `settings.yml` file in the root directory of your app Sentryms-based project, and populate it with your parameter names and test values. For example, using a parameters section like:
...
The App Sentryms is currently setup for testing with [Jasmine](http://jasmine.github.io/) (testing framework) and [Karma](https://karma-runner.github.io) (test runner). To run specs, run
...
External dependencies are defined in the  [`webpack.config.js`](https://github.com/zendesk/app_sentryms/blob/master/webpack.config.js) as `externalAssets`. This ensures these dependencies are included on your app's `index.html` as well as in the test suite.
...
## Bugs
Submit Issues via [GitHub](https://github.com/zendesk/app_sentryms/issues/new) or email support@zendesk.com.
